...
#define ProgName	"Lab2-5611-Task3"
#define LabNo		2
#define ModuleNo	5611
#define TaskNo		3
...
int main(int argc, char *argv[])
{
	...
	// Ask what 'colour' to set it to:
	if (argc >= 3) { // 'Colour' is given as command-line arg
		sscanf(argv[2], "%d", &targetColour);
		if ((targetColour < 0) || (targetColour > 255)) { // Outside valid range
			printf("Number out of range - try again\n");
			my_exit(-1);
		}
	}
	else { // Get from keyboard
		printf("\nWhat intensity value should the pixels be set to?\n");
		while (1) {	// Loop 'for ever' - it will break from inside.
			printf("Enter a number from 0 to 255 inclusive:\n");
			scanf("%d", &targetColour);
			if ((targetColour >= 0) && (targetColour <= 255)) // Valid value
				break;
			printf("Number out of range - try again\n");
		} // while
	}
	printf("Target 'colour' %d\n", targetColour);


	/* ****************************** Heart of the program ************************** */

	/* This needs to scan through all the rows and columns to visit each pixel;
	probably it will do this with two concentric loops.
	At each pixel it needs to take a decision whether to change the pixel value to
	something else (e.g. black) or leave it alone.
	If it changes it, note that what it must put there is targetLoc, i.e. the location
	in the colour map of the target colour. Do not put targetColour into the image
	(though with the simple colour map of black.bmp these are the same value).

	In addition to the above (which applied to the first two problems), you need to find
	the value of targetLoc. This must be the value which represents the colour chosen
	by the user. Their choice is in a variable targetColour - code already written
	for you will request this from the keyboard.
	Your task is to search the colour palette for an entry with colour targetColour.
	You must make targetLoc the location of this.
	(For instance, the previous programs always requested targetColour to be 0 (black),
	and in bullet.bmp this is in fact in location 0 of the palette, so targetLoc was 0.)

	Your code should allow for the case the the chosen colour cannot be found, and
	exit the program with a call to my_exit(-1); .
	*/
	//Find Target
	int Target = 500;
	for (int L = 0; L <= 255; L++) { //L is to check all elements of each pallet
		//check if R G B is equal to target colour
		if (colourPalette[L][0] == targetColour && colourPalette[L][1] == targetColour && colourPalette[L][2] == targetColour) {
			Target = L; // If found set Target
		}
	}
	if (Target == 500) {
		my_exit(-1);
	...
	int borderSize = 50;//border size
	//other declares
	int imageSize = colsPadded * DibHeader.height; //Image size usings padded columns and height
	int currentPixel = 0; //Current pixel counter
	//Bottom border
	int TBottomPixelsCh = colsPadded * borderSize; //Total pixels to set for bottom
	for (int x = 0; x < TBottomPixelsCh; x++) {
		image[x] = Target; //set pixels
	}
	//Top Border
	int TTopPixelsCh = colsPadded * borderSize; //Total pixels to set for top
	int startTopGrey = imageSize - TTopPixelsCh; //Where to start pixels set
	for (int z = startTopGrey; z < imageSize; z++) {
		image[z] = Target;//set pixels
	}
	//Left Border
	int nextLine = colsPadded - borderSize; //works out how many needed to go to new line once appropriate pixels are changed (typewriter method)
	currentPixel = 0; //Reset counter
	for (int y = 0; y < DibHeader.height; y++) { //iterate through rows
		for (int y2 = 0; y2 < borderSize; y2++) { //iterate through columns and set only columns needed
			image[currentPixel] = Target; //set pixels
			currentPixel++; 
		}
		currentPixel = currentPixel + nextLine; // next line
	}
	//Right Border
	currentPixel = nextLine-1; //Current pixel
	for (int J = 0; J < DibHeader.height; J++) { //iterate through rows
		for (int J2 = 0; J2 < borderSize; J2++) { //iterate through columns and only set columns needed
			image[currentPixel] = Target; //set pixels
			currentPixel++; 
		}
		currentPixel = currentPixel + nextLine;//next line
