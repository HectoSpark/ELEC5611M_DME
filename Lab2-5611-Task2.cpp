...
#define ProgName	"Lab2-5611-Task2"
#define LabNo		2
...
int main(int argc, char *argv[])
{
	...
	// The next line is part of the solution done for you:
	targetLoc = 0;		/* This represents black - if it is put in any
						element of the array image[], that pixel will be black. Valid for bullet.bmp
						but not for all files; normally one should search the Colour Palette.
						This value of 0 is good enough for all Lab 2 3612 Tasks and
						for Lab2-5611-Task1 and Task2, but not for Lab2-5611-Task3. */

						/* Your algorithm here needs to scan through all the rows and columns to visit each pixel;
						...
						in the colour map of the target 'colour'. Do not put the actual 'colour' (which for this
						problem is actually a shade of grey) into the image
						(though for bullet.bmp they happen to be the same number).
						*/
	//Set radius
	int radius = 50;
	//Set centre of circle X and Y
	int centreXActual = 51; 
	int centreYActual = 51; 
	//Minus 1 for later on in code
	int centreX = centreXActual - 1;
	int centreY = centreYActual - 1;
	//Image size 
	int imageSize = colsPadded*DibHeader.height;
	//first calculate circle x's and y's using pythagoras theorem and output to array
	int rad2 = radius * radius; //radius squared for pythagoras theorem
	int xArray[10000]; //large array to store x values
	int yArray[10000]; //large array to store y values
	int arrayCounter = 0; //counter for array
	for (int xc = -radius; xc <= radius; xc++) {
		for (int yc = -radius; yc <= radius; yc++) {
			int r = (yc*yc) + (xc*xc); //calculate r (hypontonuse) and check if it is the same as radius
			if (r < rad2) { //check if same as radius
				xArray[arrayCounter] = xc + centreX; //add x to array and offset 
				yArray[arrayCounter] = yc + centreY; //add y to array and offset
				arrayCounter++; //add to array counter
			}
		}
	}
	int y; //define x
	...
	for (int pixelSelected = 0; pixelSelected < imageSize; pixelSelected++) {
		//Calculate X and Y coordinates
		y = pixelSelected / colsPadded;
		x = pixelSelected - (y * colsPadded);
		//Calculate circles and check
		//check if X and Y are in the arrays
		for (int ar = 0; ar <= arrayCounter; ar++) {
			if (xArray[ar] == x && yArray[ar] == y) {//if in array turn the pixel black
				image[pixelSelected] = targetLoc; 
				//printf("\tX:\t\t%d\tY:\t\t%d\n", x, y); for debug
			}
		}
