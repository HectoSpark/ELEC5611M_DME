...
#define ProgName	"Lab3-5611-Task3"
#define LabNo		3
...
int main(int argc, char *argv[])
{
	...
	/* Your algorithm here needs work through the colour palette modifying colours.
	*/
	//find green
	int Target = 500;
	float greenRatio = 0.00f;
	float prevGreenRatio = 0.00f;
	//iterate through pallet
	for (int L = 0; L <=255 ; L++){
		float R = colourPalette[L][0]; //get value for red
		float B = colourPalette[L][2]; //get value for blue
		float G = colourPalette[L][1]; //get value for green
		float RPB = (R + B);
		greenRatio = (G / RPB); //calculate green ratio

		//Look for pure green in pallet
		if (RPB == 0 && G > 10) {
			greenRatio = G;
			if (greenRatio > prevGreenRatio) {
				Target = L; // If found set Target colour
				prevGreenRatio = greenRatio;
			}
		}
	...
	//Right Border
	currentPixel = nextLine; //Current pixel
