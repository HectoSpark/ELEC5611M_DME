...
#define ProgName	"Lab4-5611-Task1"
#define LabNo		4
...
typedef unsigned char		ColourPaletteType[256][4];

int FindEntryNearColour(int R, int G, int B, int dist,
	int coloursUsed, ColourPaletteType colourPalette)
	/* Searches colourPalette trying to find a colour within dist of the R,G,B
	specified. This means that all three colours are no more than dist from
	their desired values.
	If it succeeds, it returns the index into the palette, i.e. the number of
	the entry where that colour exists.
	If it fails, it returns -1 as a not-found signal. */
{
	int	entry;
	for (entry = 0; entry < coloursUsed; entry++)
		if ((abs(colourPalette[entry][0] - B) <= dist)
			&& (abs(colourPalette[entry][1] - G) <= dist)
			&& (abs(colourPalette[entry][2] - R) <= dist)
			)	return entry;	// Found

	return -1;	// Not found
} // FindEntryNearColour()

unsigned char FindEntryNearestColour(int R, int G, int B,
	int coloursUsed, ColourPaletteType colourPalette)
	/* Find the entry in colourPalette which is nearest to the colour specified by R,G,B.
	If none such is found, print an error message and terminate the program. */
{
	const int	Debug = 1;
	int	dist, entry;
	for (dist = 0; dist <= 255; dist++)
		if ((entry = FindEntryNearColour(R, G, B, dist, coloursUsed, colourPalette)) >= 0) {
			if (Debug) {
				printf("Colour for graph:\tsought R=%3d, G=%3d, B=%3d\n", R, G, B);
				printf("Nearest found:\t\twith   R=%3d, G=%3d, B=%3d\n",
					colourPalette[entry][2], colourPalette[entry][1], colourPalette[entry][0]);
				printf("\t\t\tentry %3d, %d away.\n", entry, dist);
			} // if
			return (unsigned char)entry;
		} // if

	printf("FATAL: can't find colour R=%d, G=%d, B=%d\n", R, G, B);
	my_exit(-1);
} // FindEntryNearestColour()
...
int main(int argc, char *argv[])
{
	...
	{ // To allow some more local definitions.

		int             i;
		long int        dummy_histogram[256];

		// Find a palette entry with the colour chosen for the histogram:
		

		// Create the dummy histogram:
		for (i = 0; i < 255; i++)
			dummy_histogram[i] = i * 1000;
	
		// Main part 1: Collect the histogram:
		// Initialise it to zero:
		long int histogram[256] = { 0 }; //initialise array
		/* Examine every pixel, finding its brightness level and incrementing the histogram
		entry for that level: */
		for (i = 0; i <= 255; i++) { histogram[i] = 0; }
		int largestHistoEntry = 0;
		int totalPixels = DibHeader.height * colsPadded;


		for (i = 0; i <= 255; i++) {
			//find monoValue
			int monoValue = (colourPalette[i][0] + colourPalette[i][1] + colourPalette[i][2]) / 3;
			//Go through each pixel and check if colour of i
			for (int currentPixel = 0; currentPixel < totalPixels; currentPixel++)
				//if colour then count at monoValue
				if (image[currentPixel] == i) { 
					histogram[monoValue]++; 
		...
		for (i = 0; i <= 255; i++) {
			if (histogram[i] > largestHistoEntry) {
				largestHistoEntry = histogram[i];
			}
		}
		//scale values by dividing by max high
		int histogramScaled[256];
		for (i = 0; i <= 255; i++) {
			histogramScaled[i] = (float) (((float) histogram[i] / (float) largestHistoEntry) * maxPixelHigh);
		}
		//Get histogram data
		...
		for (int x = 0; x <= maxPixelHigh; x++) {
			for (int y = 0; y <= 255; y++) {
				if (histogramScaled[y] >= x) {
					image[pixelCounter] = HistoColour;
				}
				pixelCounter++;
			}
			//next line
