...
#define ProgName	"Lab3-5611-Task1"
#define LabNo		3
...
int main(int argc, char *argv[])
{
	...
	/* This needs to scan through all colour map entries and replace their values
	by monochrome ones.
	*/
	nAlreadyMono = 0;
	for (colour = 0; colour < coloursUsed; colour++) {
		int	monoValue;

		/* The following IF statement counts the number of colours already
		monochrome, for information and to illustrate the use of the colour
		palette.
		This palette is really a 2-D array, but unfortunately C implements
		these rather inelegantly as a 1-D array of 1-D arrays. colourPalette
		is a 256-element array of 4-element arrays. These 'inner' 4-element arrays
		each hold the contents of one entry in the colour palette: Blue, Green,
		Red and the unused padding entry. The 'outer' array, colourPalette, has
		space for 256 of these, though the file you read may not use all.
		The variable coloursUsed has already been set to the number used.
		Remember that C array elements count from zero up. Thus if there are
		four colours used (coloursUsed=4), then elements colourPalette[0] to
		colourPalette[3] inclusive will be used.
		The actual IF statement below just checks if all three entries
		(ignoring the padding one) are the same: if so, a count nAlreadyMono
		is incremented. */

		monoValue = (colourPalette[colour][0] + colourPalette[colour][1] + colourPalette[colour][2])/3;
		colourPalette[colour][0] = monoValue;
		colourPalette[colour][1] = monoValue;
		colourPalette[colour][2] = monoValue;

		if ((colourPalette[colour][0] == colourPalette[colour][1])
			&& (colourPalette[colour][0] == colourPalette[colour][2])
			) {
			nAlreadyMono++;
		}
	...
	printf("%d of the %d original colours were already monochrome\n",
		nAlreadyMono, coloursUsed);
